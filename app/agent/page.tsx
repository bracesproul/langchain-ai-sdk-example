...
export default function Page() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl py-12 flex flex-col stretch gap-3">
      <form onSubmit={handleSubmit} aria-disabled={isLoading} className="flex flex-col gap-2">
        <input
      ...
      <div
        ...
        {data.map((item, i) => (
          <div key={i} className="p-4 bg-gray-100 rounded-lg">
            ...
            <br />
            <strong>Data:</strong> <p className="break-all text-sm">{JSON.stringify(item.data, null, 2)}</p>
          </div>
        ))}
      </div>
      {data.length > 1 && (
